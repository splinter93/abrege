...
**Version** : 1.1.0  
**Date** : 2025  
**Public cible** : Agents LLM implémentant le parser dans Scrivia et Synesia ; tiers souhaitant consommer l’API ou réimplémenter le serveur.  
...
*(La version côté API/doc peut être 1.1.0 ; le microservice Railway renvoie souvent `1.0.0` dans `/health`.)*
...
interface ParseSuccessResponse {
  ...
  data: {
    ...
    stats: {
      ...
      splitByPage: boolean;
      preset: string;
      /** true si PDFPlumber était indisponible (texte seul retourné) */
      degraded?: boolean;
      /** Raison du mode dégradé (ex. "PDFPlumber service unavailable") */
...
| **400** | Requête invalide | Fichier manquant, `result_type` invalide, fichier trop volumineux |
| **500** | Erreur serveur | Erreur interne du service |
| **502** | Service downstream KO | Proxy : `HYBRID_PARSER_URL` non configuré ou service Railway injoignable |
| **504** | Timeout | PDF trop volumineux ou service surchargé (timeout 90s) |
...
Le proxy Next.js attend jusqu'à **90 secondes** l’appel au service Railway (PDF volumineux). Côté Vercel, la fonction est limitée à **60s** (maxDuration). Recommandation client : **60 à 90 secondes**.

```typescript
const controller = new AbortController();
const timeoutId = setTimeout(() => controller.abort(), 90000); // 90s (aligné serveur)
...
try {
  ...
  if (error.name === 'AbortError') {
    // Timeout - gérer l'erreur (réponse serveur = 504)
  }
}
```
...
#### 1. "No file provided" / "Missing file field"

**Cause** : Fichier manquant dans le FormData ou champ non nommé `file`.

**Solution** :
```typescript
const formData = new FormData();
formData.append('file', file); // ✅ Champ obligatoire : "file"
```

#### 1b. "File exceeds maximum size limit"

**Cause** : Fichier > 50MB (limite du service Railway, configurable via `MAX_FILE_SIZE`).

**Solution** : Réduire la taille ou découper le PDF ; valider côté client avant envoi.
...
**Solution** :
- Timeout côté proxy : 90s ; côté client, utiliser 60–90s
- Vérifier la taille du fichier avant l'envoi (< 50MB)
- Utiliser `include_tables=false` pour accélérer
- En cas de timeout, le serveur renvoie **504** (Gateway Timeout)
...
// Ou après un parse réussi :
if (result.data?.stats?.degraded) {
  console.warn('Parsing en mode dégradé:', result.data.stats.degradedReason);
}
```

**Comportement** : Le parsing continue mais sans tableaux. La réponse inclut `stats.degraded: true` et `stats.degradedReason` (ex. `"PDFPlumber service unavailable"`). Le service peut aussi envoyer les headers `X-Parser-Degraded: true` et `X-Parser-Degraded-Reason`.
...
async function processPDFDocument(file: File, context: AgentContext) {
  ...
  const validPresets = ['default', 'insurance', 'invoice', 'contract', 'scientific'] as const;
  const preset = (typeof context.documentType === 'string' && validPresets.includes(context.documentType as typeof validPresets[number]))
...
| Limite | Valeur | Notes |
|--------|--------|-------|
| **Taille max fichier** | 50MB | Défaut service Railway ; au-delà → 400 "File exceeds maximum size limit" |
| **Timeout proxy → Railway** | 90s | Côté Next.js (FETCH_TIMEOUT_MS) |
| **Timeout fonction Vercel** | 60s | maxDuration ; au-delà → 504 côté client |
...
3. Timeout côté client : 60–90s (serveur attend 90s)
4. Split le PDF en plusieurs parties si possible
5. En cas de 504 : retry avec backoff ou fichier plus petit
...
### Code Source (côté Factoria)

- **Route Next.js (proxy)** : `src/app/api/pdf/hybrid-parse-v4/route.ts`
- **Service Railway (microservice)** : `services/hybrid-parser/` (Express, voir `src/server.ts`, `src/hybridParser.ts`)
- **Types TypeScript** : `src/lib/services/types/hybridPdfParser.types.ts`

### Implémenter le serveur (pour un tiers)

Pour déployer un clone du **microservice** Hybrid Parser (sans le proxy Next.js) :

1. **Code** : `services/hybrid-parser/` — endpoints exposés :
   - `GET /health` : retourne `{ status, services: { pdfParse, pdfPlumber }, version }` (version peut être `1.0.0` côté service)
   - `POST /parse` : query `result_type`, `split_by_page`, `preset`, `include_tables` ; body `multipart/form-data` avec champ `file`

2. **Variables d'environnement** (voir `services/hybrid-parser/src/config.ts`) :
   - `PORT` : port du serveur (défaut 8080)
   - `PDF_PARSE_URL` : URL du service pdf-parse (ex. Railway)
   - `PDFPLUMBER_URL` : URL du service pdfplumber (ex. Railway)
   - `MAX_FILE_SIZE` : taille max en octets (défaut 50 * 1024 * 1024)
   - `HYBRID_TIMEOUT_MS` : timeout des appels aux sous-services (défaut 25000)

3. **Dépendances** : le microservice appelle deux services externes (pdf-parse et pdfplumber) ; ils doivent être déployés et accessibles (voir `services/pdf-parse/` et `services/pdfplumber/`).

4. **Proxy Next.js** : si tu utilises le proxy Factoria, définir `HYBRID_PARSER_URL` vers l’URL publique du microservice (ex. `https://hybrid-parser.up.railway.app`).
...
- [ ] ✅ Gestion des timeouts (60–90s côté client)
