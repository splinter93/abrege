...
export default function AuthPage() {
  ...
  const handleOAuthSignIn = async (provider: 'google' | 'github' | 'apple') => {
    ...
    console.log(`🔧 OAuth ${provider} - URL de redirection:`, redirectUrl);
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('not configured') || error.message.includes('not enabled')) {
          setError(`${provider.charAt(0).toUpperCase() + provider.slice(1)} OAuth n'est pas encore configuré. Utilisez l'authentification par email pour le moment.`);
        } else {
          setError(`Erreur ${provider}: ${error.message}`);
        }
        setLoading(false);
      }
      // Si pas d'erreur, l'utilisateur sera redirigé vers le provider OAuth
    } catch (error: any) {
      console.error(`Unexpected OAuth ${provider} error:`, error);
      setError('Erreur inattendue lors de la connexion OAuth');
      setLoading(false);
    }
  };
