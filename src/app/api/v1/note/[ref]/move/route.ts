...
// Méthode GET pour tester l'accessibilité de l'endpoint
export async function GET(
  ...
  return new Response(
    JSON.stringify({ 
      message: 'Endpoint move accessible',
      ref,
...
export async function PATCH(
  ...
  try {
    ...
    if (!paramResult.success) {
      return new Response(
        JSON.stringify({ 
          error: 'Paramètre note_ref invalide', 
          details: paramResult.error.errors.map(e => e.message) 
        }),
    ...
    if (body.target_classeur_id) {
      try {
        ...
        return new Response(
          JSON.stringify({ 
            error: `Notebook cible '${body.target_classeur_id}' introuvable ou non accessible.` 
          }), 
          { status: 404, headers: { 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (body.target_folder_id !== undefined) {
      if (body.target_folder_id === null || body.target_folder_id === '' || body.target_folder_id === 'null') {
        ...
        try {
          ...
          return new Response(
            JSON.stringify({ 
              error: `Dossier cible '${body.target_folder_id}' introuvable ou non accessible.` 
            }), 
            { status: 404, headers: { 'Content-Type': 'application/json' } }
          );
        }
    ...
    if (!updated) {
      return new Response(
        JSON.stringify({ 
          error: 'Aucune note mise à jour (slug/id incorrect ?)' 
        }), 
        { status: 404, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new Response(
      JSON.stringify({ 
        error: err instanceof Error ? error.message : 'Erreur inconnue' 
      }), 
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
