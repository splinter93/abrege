  ...
  return {
    ...
    '/api/v2/note/{ref}/content:apply': {
      post: {
        ...
        description: 'Applique des opérations de contenu sur une note (insert, replace, delete, upsert_section). Si un canva avec status=\'open\' existe pour cette note, le contenu est streamé progressivement vers le client. Toujours sauvegardé en DB.',
        tags: ['Notes'],
        security: [
          { BearerAuth: [] },
          { ApiKeyAuth: [] }
                      ...
                      properties: {
                        data: {
                          ...
                          properties: {
                            ...
                            diff: { type: 'string' },
                            content: { type: 'string' },
                            streaming_enabled: { type: 'boolean', description: 'True si le contenu a été streamé vers un canva ouvert' },
                            saved_to_db: { type: 'boolean', description: 'Toujours true — la note est persistée en DB après chaque appel' }
                          }
                        },
                        meta: {
                          type: 'object',
    ...
    // editNoteContent supprimé — fusionné dans content:apply
  };
}
