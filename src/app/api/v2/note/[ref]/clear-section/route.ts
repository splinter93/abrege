...
export async function DELETE(
  ...
  const context = {
    operation: 'v2_note_clear_section',
    component: 'API_V2',
    ref,
    clientType
  };

  logApi('v2_note_clear_section', `🚀 Début vidage section note v2 ${ref}`, context);

  // 🔐 Authentification
  const authResult = await getAuthenticatedUser(request);
  if (!authResult.success) {
    logApi('v2_note_clear_section', `❌ Authentification échouée: ${authResult.error}`, context);
  ...
  if (!permissionResult.success) {
    logApi('v2_note_clear_section', `❌ Erreur vérification permissions: ${permissionResult.error}`, context);
    return NextResponse.json(
  ...
  if (!permissionResult.hasPermission) {
    logApi('v2_note_clear_section', `❌ Permissions insuffisantes pour note ${noteId}`, context);
    return NextResponse.json(
  ...
  try {
    ...
    // Validation Zod V2
    const validationResult = validatePayload(clearSectionV2Schema, body);
    if (!validationResult.success) {
      logApi('v2_note_clear_section', '❌ Validation échouée', context);
    ...
    if (fetchError || !currentNote) {
      logApi('v2_note_clear_section', `❌ Note non trouvée: ${noteId}`, context);
      return NextResponse.json(
    ...
    logApi('v2_note_clear_section', `🔍 Sections disponibles:`, toc.map(t => ({ title: t.title, slug: t.slug })), context);
    logApi('v2_note_clear_section', `🔍 Section recherchée: "${validatedData.sectionId}"`, context);
    ...
    let newContent = clearSection(currentNote.markdown_content || '', validatedData.sectionId);
    ...
    if (updateError) {
      logApi('v2_note_clear_section', `❌ Erreur mise à jour: ${updateError.message}`, context);
      return NextResponse.json(
    ...
    logApi('v2_note_clear_section', `❌ Erreur serveur: ${error}`, context);
    return NextResponse.json(
