  ...
  try {
    ...
    if (envApiKeys && defaultUserId) {
      ...
      if (validKeys.includes(apiKey)) {
        logApi.info(`✅ API Key validée via variables d'environnement: ${apiKey}`, { 
          component: 'AuthUtils',
          authType: 'env_api_key'
        });
    ...
    logApi.error(`❌ Erreur validation API Key: ${error}`, { component: 'AuthUtils', error });
    return null;
  }
} 